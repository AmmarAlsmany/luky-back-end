...
// DONUT UPDATE
//
var colors = ["#22c55e", "#ef4444", "#7f56da"]; // accepted, cancelled, pending

// random-ish numbers (inline, no helper function)
const total = Math.floor(300 + Math.random() * 900); // 300–1200
const accepted = Math.floor(total * (0.55 + Math.random() * 0.35)); // 55–90% of total
const cancelled = Math.min(Math.floor(total * (Math.random() * 0.30)), total - accepted); // up to 30%
const pending = Math.max(total - accepted - cancelled, 0);
...
var options = {
  ...
  series: [accepted, cancelled, pending],
  colors: colors,
  legend: {
    show: true,
    position: 'bottom',
...
var chart = new ApexCharts(
  document.querySelector("#request-donusts"),
  options
);
chart.render();
//
